"""
Usage:
    python 1_convert_to_parquet.py \
...
def main():
    ...
    meta_parquet = str(out / "meta_raw.parquet")
    reviews_parquet = str(out / "k_reviews_raw.parquet")
    ...
    con.execute(f"""
        ...
        TO '{meta_parquet}' (FORMAT PARQUET, COMPRESSION ZSTD)
    """)
    ...
    con.execute(f"""
        ...
        TO '{reviews_parquet}' (FORMAT PARQUET, COMPRESSION ZSTD)
    """)
    ...
    print(f"[SAVED] meta_raw -> {meta_parquet}")
    print(f"[SAVED] reviews_raw -> {reviews_parquet}")
